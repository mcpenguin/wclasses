...
export async function getServerSideProps(context: any) {
  ...
  let termOfferingsResponse = await axios.get(
    `http://localhost:8000/courses/termOfferings/${subjectCode.toUpperCase()}/${catalogNumber.toUpperCase()}/`
  );
  const termOfferingsDetails: string[] = (await termOfferingsResponse).data;

  let corequisitesAsCodes: undefined | ICourseCode[] =
  ...
  let antirequisitesAsCodes: undefined | ICourseCode[] =
    courseDetails.antirequisites;
  let antirequisites: ICourseWithUWFlowData[] = [];
  if (antirequisitesAsCodes) {
    antirequisites = await GetCourses(antirequisitesAsCodes);
  }

  let postrequisitesAsCodes: undefined | ICourseCode[] =
    courseDetails.postrequisites;
...
const Schedule: NextPage = (props: any) => {
  ...
  let termCodesAsElements = termOfferingsDetails.map((t) => (
    <li key={t}>{parseTermCode(t)}</li>
  ));
