...
const AddUWFlowInfoToCoursesFromDB = async (
  courses: ICourse[]
): Promise<any> => {
  let result = [];
  for (const course of courses) {
    const dataAboutCourseFromUWFlow: IUWFlowCourse =
      await GetInfoForCourseFromUWFlow(
        course.subjectCode,
    ...
    const dataAboutCourse = {
      ...
      postrequisites: dataAboutCourseFromUWFlow?.postrequisites?.map(
        (postreq) => splitCourse(postreq.postrequisite.code)
      ),
...
// courses: ICourseCode[]
export const GetInfoForCourses = async (
  req: Request,
  res: Response
): Promise<void> => {
  ...
  try {
    if (courses.length === 0) {
      res.status(500).json({ err: "No courses specifed" });
      return;
    }
    const dataAboutCoursesFromDB: null | ICourse[] = await Course.find({
      $or: courses,
    });
    const coursesWithUWFlowData: ICourseWithUWFlowData =
      await AddUWFlowInfoToCoursesFromDB(
